...
class WCGS_Products {
    ...
    public static function sync($syncable_data) {
         ...
         **/
        $syncable_data = apply_filters('wcgs_sync_data_products_before_processing', $syncable_data);
        
        // wcgs_log($syncable_data); exit;
        
        
        $variations = array_filter($syncable_data, function($row){
            return $row['type'] == 'variation' && ! empty($row['parent_id']);
        });
        
        $simple_prods = array_filter($syncable_data, function($row){
        ...
        $wcapi_data = [];
        // Existing data
        $wcapi_data['update'] = array_filter($simple_prods, function($row){
                        return $row['id'] != '';
            });
        // New data
        $wcapi_data['create'] = array_filter($simple_prods, function($row){
                        return $row['id'] == '';
            });
